...
const updateShop = async (userId, shopId, updateData) => {
  ...
  if (
    ...
    const currentListedCards = await shopRepository.findListedUserCardsByShopId(
      shopId,
    );
...
const getShopDetail = async shopId => {
  ...
  const listedCards = await shopRepository.findListedUserCardsByShopId(shopId);
...
const findShopById = async shopId => {
  return await shopRepository.findShopById(shopId);
};
